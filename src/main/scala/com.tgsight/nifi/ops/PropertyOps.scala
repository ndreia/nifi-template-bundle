...
class PropertyValueOps(val pv: PropertyValue) extends AnyVal {
  ...
  @nowarn def evaluate(flowFile: FlowFile)(implicit context: ProcessContext): PropertyValue =
    pv.evaluateAttributeExpressions(flowFile)

  @nowarn def get(implicit context: ProcessContext): String =
    pv.getValue
...
class PropertyDescriptorValue(name: String, prop: PropertyValue) {
  ...
  def getEither: Either[String, String] =
    getValue(Right(_), Left(name + "is not set"), Left(name + " is null"))
